...
residuals = zeros(runs,1); % Store for the metrics of the outcomes.
popfin = zeros(runs,2);
IVPs = linspace(0,0.19,runs); % Initial conditions to be run.
...
for run = 1:runs % Loop on the different initial conditions.
    ...
    N0(4) = 0; % Free predator corresponding to the S0 of the run.
    N0(5) = IVPs(run); % No biofilm predators.
    ...
    residuals(run,1) = F - 1.730; % Get -> (freecells - expected(freecells))
    residuals(run,2) = B - 0.0; % Get -> (biofilmcells - expected(biofilmcells))
...
N0(5) = 0;
[F,B] = solvePop(N0,t0,t1);
pops(1,:) = [F B]; % top figs, total prey composition.

% Rerun the ODE system with the best conditions.
N0(5) = IVPs(I);
[F,B] = solvePop(N0,t0,t1);
pops(2,:) = [F B]; % top figs, total prey composition.
