...
% User input
gammas = 0; % Values: 0 OLS, 1 WLS, other GLS % Set the LS method
...
while ii<maxits && parchange>partol && oldparchange>partol || ii<minits
    ...
    % Solver inputs 
    gls_error_estimate = @(params)gls_formulation(params,gls_optpar(:,2),park,data,ts,weights);
    options = optimset('MaxIter',2500,'MaxFunEvals',5000,'Display', 'Off');

    % Get the parameters iteratively
    gls_optpar(:,1) = fminsearch(gls_error_estimate,gls_optpar(:,1),options);
    ...
    for i = 1:nparams
        ...
        if ~isempty(a); par(i) = gls_optpar(a,1);          % If found, implement
    ...
    [~,weightstemp] = ode45(@odefun,ts,y0,opts,par); 
    ...
    parchange = ...
        (1/2)*sum((abs(gls_optpar(inds)-old_gls_optpar(inds))./old_gls_optpar(inds)));
...
function resid = gls_formulation(paru1,paru2,park,data,ts,weights)

nparams = 31;
par = zeros(nparams,1); % Store for the parameter array for the ODE solver
% Extract the parameters in the correct order
for i = 1:nparams
    a = find( paru2(:,1) == i ); % Look for i'th parameter in unknown param array
    if ~isempty(a); par(i) = paru1(a,1); % If found, implement
    else; par(i) = park(park(:,2) == i,1); % If not found, implement from known param array
    end
end
